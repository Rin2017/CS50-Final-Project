...
function addMarker(place)
{
    ...
    //keep track of markers in an array
    markers.push(marker);
    
    // get articles for each zip code
    $.getJSON(Flask.url_for("articles"), {geo: place.postal_code}, function(articles) {
        
        // create html list by iterating over each articles
        var list = "<ul>";
        for (var i = 0; i < articles.length; i++)
        {
        	list += "<li><a href='" + articles[i].link + "'>" + articles[i].title + "</a></li>";
        }
		list += "</ul>";
		
		// listen for clicks on marker and show the articles list
